import { Game, QuestionSelector, QuestionSelectionStrategies, GameQuestion, Question } from './models';
...
export class GameFactory {
  public static getRandom = async (strategy: String): Promise<Game> => {
    const questionSelectionStrategy = GameFactory.getQuestionSelectionStrategy(strategy);

    do {
      const randomWord = GameFactory.getRandomWord();
      const questionSelectors = GameFactory.getQuestionSelectors(questionSelectionStrategy, randomWord);
      
      if (randomWord.length === questionSelectors.length) {
        let gameQuestions = GameFactory.getRandomQuestions(questionSelectors).map((question, index) => new GameQuestion(index + 1, index + 1, question));
        GameFactory.shuffle(gameQuestions);
        gameQuestions = gameQuestions.map((gameQuestion, index) => new GameQuestion(index + 1, gameQuestion.wordPosition, gameQuestion));
  ...
  public static getPossibleQuestions = (selector: QuestionSelector) : Question[] => {
     return GameCache.QuestionsByFirstLetterAnswer.get(selector.firstLetterAnswer)?.filter((q) => q.category === selector.category && q.level === selector.level) ?? [];
  }

  private static getQuestionSelectionStrategy = (strategy: String) => {
    if (strategy === null || strategy === undefined) return QuestionSelectionStrategies.Random;

    switch (strategy.toUpperCase()) {
      case QuestionSelectionStrategies[QuestionSelectionStrategies.RandomOnlyEasy].toUpperCase():
  ...
  private static getQuestionSelectors = (strategy: QuestionSelectionStrategies, word: String) => {
