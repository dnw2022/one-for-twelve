import { QuestionSelectionStrategies, QuestionCategories, QuestionSelector } from './models';
...
class RandomQuestionSelectionStrategy extends QuestionSelectionStrategy {
    ...
    for (const letter of letters) {
      ...
      const selector = new QuestionSelector(letter.toUpperCase(), level, category);
      const firstLetterAnswer = letter.toUpperCase();
      
      const questions = GameCache.QuestionsByFirstLetterAnswer.get(firstLetterAnswer) ?? [];

      const possibleQuestions = questions.filter((q) => q.category === category && q.level === level);

      if (possibleQuestions.length === 0) {
        console.log(`No questions with: ${firstLetterAnswer}, ${category}, ${level}`);
        break;
      }
