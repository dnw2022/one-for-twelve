import { Question, QuestionCategories, CacheStats } from './models';
...
export class GameCache {
    ...
    private static questionsByFirstLetterAnswer: Map<String, Question[]> | null = null;
  
    static get Words() : String[] { return GameCache.words ?? [] };
    static get QuestionsByFirstLetterAnswer() : Map<String, Question[]> { return GameCache.questionsByFirstLetterAnswer ?? {} as Map<String, Question[]> };
  
    public static Init = async (wordFiles: String[], questionFiles: String[]) : Promise<void> => {
    ...
    public static getStats = (): CacheStats => {
      const questionCount = [...GameCache.QuestionsByFirstLetterAnswer.values()].reduce((acc, cur) => acc+cur.length, 0);  
      return new CacheStats(GameCache.Words.length, questionCount);
    }
    ...
    private static cacheQuestions = async (files: String[]) : Promise<void> => {
      return new Promise(async (resolve, reject) => { 
        ...
        const questions = ([] as Question[]).concat(...await Promise.all(promises));
        GameCache.questionsByFirstLetterAnswer = GameCache.groupBy(questions, (q: Question) => q.firstLetterAnswer);
      ...
      return new Promise((resolve, reject) => {  
        ...
        fs.createReadStream(file)
          ...
          .transform((row: any, next: any) => {
              return next(null, Question.createText(
                row.number,
